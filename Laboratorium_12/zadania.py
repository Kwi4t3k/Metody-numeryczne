...
def metoda_laguerre_jeden_pierwiastek(wspolczynniki, z0, epsilon=1e-9, max_iteracji=100):
    z = complex(z0)
...
def metoda_laguerre_wszystkie_pierwiastki(wspolczynniki, z0=0, epsilon=1e-9):
    aktualny_wielomian = wspolczynniki.copy()
    pierwiastki = []

    while len(aktualny_wielomian) > 2:
        pierwiastek = metoda_laguerre_jeden_pierwiastek(
            aktualny_wielomian,
            z0,
            epsilon
        )
    ...
    ostatni_pierwiastek = -aktualny_wielomian[1] / aktualny_wielomian[0]
    pierwiastki.append(ostatni_pierwiastek)
...
def testuj_wielomian(nazwa, wspolczynniki):
    ...
    print("P(2) =", schemat_Hornera(wspolczynniki, z))
    ...
    jeden_pierwiastek = metoda_laguerre_jeden_pierwiastek(wspolczynniki, complex(0, 0))
    print("Pierwiastek =", jeden_pierwiastek)

    print("\nZadanie 4 - wszystkie pierwiastki metodą Laguerre'a:")
    pierwiastki = metoda_laguerre_wszystkie_pierwiastki(wspolczynniki, complex(0, 0))

    for i in range(len(pierwiastki)):
        print(f"z{i + 1} =", pierwiastki[i])
